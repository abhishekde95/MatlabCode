...
for i = 1:length(flist)
    ...
    %Loog = strcmp(stro.sum.trialFields(1,:), 'oog'); Does this make sense to hold on to?
...
for i = 1:length(thetas)
    ...
    xytmp = [x,y]*rotmat;
    [fpar,fv] = fmincon(@(params) tf_fiterr2(params,[xytmp(:,1) xytmp(:,2) tf],Loog),fpar,[],[],[],[],[LB LB],[UB UB],[],options);
    toterr(i) = fv;
    fpars(i,:) = fpar;
end
