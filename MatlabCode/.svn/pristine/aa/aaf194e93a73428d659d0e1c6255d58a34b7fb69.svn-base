...
function PrepareStim(nStixelGrid, dvaPerStixel, stimDur, cc1, cc2, cc3, seed, epoch)
    ...
    if epoch == 1 %Dense Noise
        ...
        if lumcc == 0 && colcc > 0
            Lcc = sqrt((colcc^2)/2);
            Mcc = Lcc;
            cc = [Lcc -Mcc Scc; -Lcc Mcc Scc];
        elseif colcc == 0 && lumcc > 0
            Lcc = sqrt((lumcc^2)/2);
            Mcc = Lcc;
            cc = [Lcc Mcc 0; -Lcc -Mcc 0];
        else
    ...
    elseif epoch == 2 %GLMP
        ...
        %Reassigning Lum and Chrom values to L and M vals
        L = cc1;
        M = cc2;
        S = cc3;
...
%         temp = mod(gl.stim.framecounter,4);
%         if temp == 0 || temp == 1
%             randnums = ones(size(randnums));
%         else
%             randnums = 2*ones(size(randnums));
%         end
