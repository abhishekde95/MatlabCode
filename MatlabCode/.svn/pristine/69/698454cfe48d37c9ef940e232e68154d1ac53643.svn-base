...
subunitmaskidx = strcmp(WN.sum.rasterCells,'subunit_mask');
subunitmask = WN.ras(:,subunitmaskidx);
if ~isnan(subunitmask{1})  
    L = false(ntrials,1);
    for i = 1:ntrials
        if all(subunitmask{i} == 0)
            L(i) = 1;
        end
    end
    WN.trial = WN.trial(L,:);
...
% Laser stuff
laseronidx = find(strcmp(WN.sum.trialFields(1,:),'laser_on'));
laseroffidx = find(strcmp(WN.sum.trialFields(1,:),'laser_off'));
laseridx = find(strcmp(WN.sum.trialFields(1,:),'laser'));
...
% Time course plot
peakelement = find(STAs(:).^2 == max(STAs(:).^2));
peakframe = ceil(peakelement/size(STAs,1));
STA = reshape(STAs(:,peakframe),[nstixperside nstixperside 3]);
peakpixel = find(sum(STA.^2,3) == max(max(sum(STA.^2,3))));
timecourse = STAs(peakpixel+[0 nstixperside^2 2*nstixperside^2],:);
figure; axes; hold on;
plot(timecourse(1,:),'r-','Linewidth',1.5);
plot(timecourse(2,:),'g-','Linewidth',1.5);
plot(timecourse(3,:),'b-','Linewidth',1.5);
plot([1 size(timecourse,2)],[0 0],'k:');
tmpidx = find(WN.sum.fileName == '/',1,'last');
title(WN.sum.fileName([tmpidx+1:end]));
%%
...
%%
% Rasters for laser and nonlaser trials
Llaser = WN.trial(:,laseridx);
figure;
stimon_t = WN.trial(:,stimonidx);
stimoff_t = WN.trial(:,stimoffidx);
laseron_t = WN.trial(:,laseronidx);
laseroff_t = WN.trial(:,laseroffidx);
spikes = WN.ras(:,spikeidx);

if any(Llaser) % only do this if there are laser trial
    for i = unique(Llaser)' % no laser, laser
        subplot(2,1,find(i==unique(Llaser))); hold on;
        counter = 0;
        ...
        for j = find(Llaser' == i)
            ...
            if i == 0
                ...
                plot([spikes{j} spikes{j}]'-laseron_t(j),[zeros(nsp,1) .5*ones(nsp,1)]'+counter,'k-')
            end
            counter = counter + 1;
        end
        title(['laser = ',num2str(i)]);
    end
    xlabel('time(s)');
end
...
%%
% Extracting latency measurements from replay experiments and white noise. 
% In a population analysis I'll look for a correlation between these two
% things
VERBOSE = 1;
...
Lsynthimg = [];
for i = 1:length(allimages)
    Lsynthimg = [Lsynthimg; ~isnan(allimages{i}(1))];
end

ntimebins = 500;
timebins_baseline = [-ntimebins:-1]/1000; % ms
timebins_driven = [1:ntimebins]/1000; % ms
PSTH_driven = zeros(1,ntimebins);
PSTH_baseline = zeros(1,ntimebins);
for i = find(Lsynthimg)'
    stimon_t = WN.trial(i,stimonidx);
    numframes = WN.trial(i,nframesidx);
    stimoff_t = stimon_t+numframes/framerate;
    spikes = WN.ras{i,spikeidx}-stimon_t; % Spike times are wrt stimulus onset
    ...
    if (VERBOSE)
        subplot(2,2,3); hold on; plot(spikes,i*ones(length(spikes),1),'k.'); % Debugging
    end
end
% PSTHs are sums across trials. Threshold is mean (of spike counts) across time.
MAXISI = 5; % Need two PSTH bins exceeding the threshold within MAXISI msecs
...
replay_lat = timebins_driven(abovethreshold(find(isi<=MAXISI,1)));
if (VERBOSE)
    subplot(2,2,4); hold on;
    plot(timebins_driven,PSTH_driven);
    plot([replay_lat replay_lat],[0 max(PSTH_driven)],'k-')
end
[replay_lat peak_t centroid_t]