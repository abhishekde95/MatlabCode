...
if gl.stim.epoch == 1 || gl.stim.epoch == 2
    ...
    if ~rem(gl.stim.drawrect(1), 2)
        ...
        gl.stim.drawrect(4) = gl.stim.drawrect(4)+1;
    else
        gl.stim.drawrect(1) = gl.stim.drawrect(1)-1;
        gl.stim.drawrect(2) = gl.stim.drawrect(2)-1;
    end
    
elseif gl.epoch == 3
    
    if (size(gl.gridX,1) < 2)
        PC_1 = [1 0];  % single pixel, pick horizonatal
    else
        coeff = pca([gl.gridX gl.gridY]);
        PC_1 = coeff(:,1);
    end
    ...
    gl.stim.sf = halfperiod*2 / gl.pixperdeg; % in cycles/deg
    gl.stim.theta = theta;
    gl.stim.sigma = .4;
    gl.stim.nfamesplateau = 26;
    ...
    x = (gl.bar.xy(1) * gl.pixperdeg) + (mean(gl.gridX) * gl.pixperstix); % in pixels
    y = (gl.bar.xy(2) * gl.pixperdeg) + (mean(gl.gridY) * gl.pixperstix);
...
elseif gl.stim.epoch == 3 % Large Gabor
    ...
    frame = gl.stim.template(:,:,gl.framecounter);
    for plane = 1:3
    ...
    if gl.stimcounter < gl.stim.nframes
        gl.fliprequest = 1;
    else
        gl.stim.on = 0;
        %Screen('Close',gl.stim.textureidx)
