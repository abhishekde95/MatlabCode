...
for ii = 1:numcells
    disp(ii);
    [out2,fittedGabor,Rsquare_Gabor] = gaborfit_AD(Output_List{ii,4}); % fitting Gabor
    [out3,fittedDOG,Rsquare_DOG] = DOGfit(Output_List{ii,4});
    
    [tmp_out1,fittedCrescent1,Rsquare_Crescent1] = Crescentfit_AD(Output_List{ii,4},out3); % Fitting the crescent shaped model, takes input from the DOG model
    [tmp_out2,fittedCrescent2,Rsquare_Crescent2] = Crescentfit_AD(Output_List{ii,4},[]);
    ...
    R_square = [R_square; Rsquare_Crescent Rsquare_Gabor Rsquare_DOG];
...
if plotresults
    ...
    % Energy
    subplot(num_rows,C,((ii-(kk-1)*num_rows)-1)*C+6); plot(Output_List{ii,7}); set(gca,'XTick',[],'YTick',[]); axis square;
    if mod(ii,num_rows) ==1
        title('Energy');
    end
