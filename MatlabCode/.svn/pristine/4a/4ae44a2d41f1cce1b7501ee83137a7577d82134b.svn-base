...
stim.exptDone = 0;
stim.exptMethod = 0;            % 1=MoCS, 2=Staircasing
...
stim.flashDeltaPhase = 0;       % set later on in the code, but initializing to [], or nan could be problematic
stim.flashMovie = [];
stim.flashMaxInc = 0;
...
%wireframe
    function wireFrameToggle(onOff, showFrames)
        ...
        stim.frameRGB = stim.frameRGB/255;
    end
        ...
        switch stim.exptMethod
            case 1 %MoCS
                for a = 1:numColors;
                    ...
                    stim.rgbLevels{a} = (ptb.invM * coneLevels)';
                end
            case 2 %staircasing
    ...
    function setupTrial(RFpos, flashLength, fpPos, fpSize, frameOffset, targSize, T1Dist, T2Dist, gaborTheta, gaborGamma, gaborSigma, gaborSpeed, nSigmas, lastTrialGood, lastTrialCorrect, trialType)
        ...
        switch trialType
            case 0 % detection trial
                ...
                row = -halfSize:2:halfSize-1; %#ok<*BDSCI> %subtract one so that you don't overlap the texture window;
    ...
    function [rgbTrial, lambda, color, sfs, contrast, targ] = MoCS(lastTrialGood, trialType)
        ...
        switch trialType
            ...
            case 1 % grating (fixation) trials
                ...
                rgbTrial = (ptb.invM * tlms)'; %in gun intensities
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [rgbTrial, lambda, color, sfs, contrast, targ] = Staircase(lastTrialGood, lastTrialCorrect)
        ...
        rgbTrial = (ptb.invM * ((1+(trialLMS'./100)) .* ptb.bkgndlms))';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [rgbTrial, lambda, color, sfs, contrast, targ] = Quest(lastTrialGood, lastTrialCorrect)
        ...
        rgbTrial = (ptb.invM * ((1+LMS)' .* ptb.bkgndlms))';
        lambda = stim.spPeriodLevels(sfs);
    end
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function setupMonitor(mondist, screenwidth, calFilePath, fitMeth)        
        %load in the calibration data and fill up the ptb structure
        
        load(calFilePath);
        calData = cals{end}; %#ok<USENS>
        switch fitMeth
            case {1,2,3} %2 deg funds with grating fits
                smj = load('T_cones_smj.mat');
                fundamentals = smj.T_cones_smj;
                fundWavelengthSpacing = smj.S_cones_smj;
            case {0,4} %10 deg funds with grating fits
                smj = load('T_cones_smj10.mat');
                fundamentals = smj.T_cones_smj10;
                fundWavelengthSpacing = smj.S_cones_smj10;
        end
        ...
        stim.targRGB = (ptb.bkgndRGB - 30)/255;
        ...
        if ~isempty(Screen('Windows'))
            ...
            Screen('FillRect', ptb.w, calData.bgColor);
        else
            ...
            ptb.w = PsychImaging('OpenWindow', 0, calData.bgColor);
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function movie = makeGaborMovie(X,Y)
        ...
        movie = zeros(1,stim.flashNumFrames);
        ...
        for a = 1:stim.flashNumFrames
            ...
            if frameMaxInc > stim.flashMaxInc;
                ...
                stim.flashRGB(4) = round(ptb.maxdac * flashRGB(r,c,1));  % DAC values b/w 0&maxdac
                stim.flashRGB(5) = round(ptb.maxdac * flashRGB(r,c,2));
                stim.flashRGB(6) = round(ptb.maxdac * flashRGB(r,c,3));
                stim.flashMaxInc = frameMaxInc;
            end
         
            %make a frame of the stimulus movie
            movie(a) = Screen('MakeTexture', ptb.w, TranslateToColourMode(flashRGB, [], ptb.ccmode), [], [], 2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function movie = makeGratingMovie()
        ...
        for i = 1:stim.flashNumFrames
            ...
            movie(i) = Screen('MakeTexture', ptb.w, TranslateToColourMode(im, [], ptb.ccmode), [], [], 2);
        end
...
%BLANK THE SCREEN
    function allOff()
        ...
        stim.gaborPhase = unifrnd(0, (2*pi)-stim.flashDeltaPhase);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function loadGaborFits(fitMethod)
        ...
        %(3) = Grating with S- and L-M colors, (4) = gratings with 10deg
        %fundamentals.
