...
for q=1:size(datafiles,1)
    ...
    if num_resamples~=0
        [th_fit_rs(:,:,q),err_rs(:,q),rs(1,:,q),Bdist(:,q),Cbdist(:,q)] = resample(datafile_eval1(:,:,q),tg(1:4),num_resamples);
    end
    
    %Plot Differences between Cb1 and Cb2
    if q==2
        Cbdiffdist = Cbdist(:,2)-Cbdist(:,1);
        figure(27); clf; hold on; grid on;
        set(gca,'FontSize',14)
        title('Distribution of Cb1 - Cb2')
        xlabel('Difference between Cb1 and Cb2')
        ylabel('Proportion of Occurances')
        [n,xout]=hist(Cbdiffdist,100);
        bar(xout,n/sum(n)*100,1)
    end
...
% One A, Tnd, B, and one k's for H and L coherences
[th_fit2_b,err2_b] = fitwithcurves2_b(df,tg(1:4))
