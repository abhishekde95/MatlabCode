function SLMTF_KMwsf() %#ok<*ASGLU,*NASGU>
% Making changes to the paradigm
% Originally designed by Zack for the LMTF_2stim paradigm
% Modified by Abhishek on 11/15 to run SLMTF
global gl
...
NCONTRASTS = 50;
TIME_POSITION = [1 2];
DIST_FROM_SCREEN = 200; % 70 -ProPixx, 100- Dell8, ViewPixx
...
gl.stim.scale_lattice = logspace(-2, 0, NCONTRASTS); % defining the contrasts here
gl.fp.xyoffset = [20 0];
gl.fp.size = FPSIZE;
gl.trials = 0; % To keep a track of trial number
gl.MAX_TRIALS = 100; % Denotes the maximum number of trials in a block of an experiment
...
gl.trial_counter = (gl.MAX_TRIALS/(2*numel(gl.stim.sf)))*ones(1,2*numel(gl.stim.sf));

gl.correct_ans_stepsize =  ones(1,2*numel(gl.stim.sf));
...
N = numel(sf);
L_chrom = repmat([0.0636; 0.0636],[N 1]);
M_chrom = repmat([-0.0636; -0.0636],[N 1]);
S_chrom = repmat([-0.45; 0.45],[N 1]);
...
gl.color_ID = cell(1,N*2);
stim_dir1 = '(L-M)-S_sf';% S-OFF
stim_dir2= '(L-M)+S_sf'; %S-ON
for ii = 1:N
    gl.color_ID{2*ii-1} = strcat(stim_dir1,num2str(ii));
    gl.color_ID{2*ii} = strcat(stim_dir2,num2str(ii));
end
...
gl.stim.cc_dt_chrom = cc_at_detection(:,1:N*2);
...
for ii = 1:N
    ...
    gl.prev_scale_idx(2*ii-1:2*ii) = start_contrast_idx;
    gl.next_scale_value(2*ii-1:2*ii) = gl.stim.scale_lattice(start_contrast_idx);
end
