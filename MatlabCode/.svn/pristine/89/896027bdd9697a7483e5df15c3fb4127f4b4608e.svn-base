...
function [out,fittedgabor,R_square,Pearson_R] = gaborfit_AD(im)
    ...
    numerator = fval; %min([sum((fittedgabor(:)+im(:)).^2) sum((fittedgabor(:)-im(:)).^2)]);
    denominator = sum(im(:).^2.*(im(:)-mean(im(:))).^2);
    R_square = 1-(numerator/denominator);
    Pearson_R = acos(abs(corr(fittedgabor(:),im(:))))*180/pi;
