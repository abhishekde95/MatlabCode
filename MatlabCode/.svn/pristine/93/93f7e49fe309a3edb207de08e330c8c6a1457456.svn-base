...
%%%%%%%%%%%%%%%%%%%%%%%%%%
data = {};
[fnames, spikeIdx] = fnamesFromTxt2([nexfilepath,filesep,'nexfilelists',filesep,'Greg',filesep,'Gratings.txt']);
for a = 1:size(fnames,1)
    stro = {};
    for i = 1:size(fnames{a},2)
        tmpstro = nex2stro(findfile(char(fnames{a}(i))));
...
%%
% Section 3.2
% Looking through the gratings data for "horseshoe" cells (rectified along
...
%%
% Section 3.3
% Looking for complex cells that have: 
% (a) strong response to L+M but not L-M
% (b) strong response to L-M but not L+M
% (c) similarly strong responses to L+M and L-M

out = zeros(length(data),5);
for i = 1:length(data)
    L_lum = sign(data(i).colors(:,1)) == sign(data(i).colors(:,2));
    L_lum = L_lum & abs(data(i).colors(:,1)) == abs(data(i).colors(:,2));
    L_lum = L_lum & data(i).colors(:,3) == 0;
    L_rg = sign(data(i).colors(:,1)) ~= sign(data(i).colors(:,2));
    L_rg = L_rg & abs(data(i).colors(:,1)) == abs(data(i).colors(:,2));
    L_rg = L_rg & data(i).colors(:,3) == 0;
    if sum(L_rg)~=1 || sum(L_lum)~=1
        continue
    end
    out(i,:) = [i data(i).F1(L_lum) data(i).F0(L_lum) data(i).F1(L_rg) data(i).F0(L_rg)];
end
...
L_lum_cmplx = out(:,2)./out(:,3) < 1;
L_rg_cmplx = out(:,4)./out(:,5) < 1;
L_cmplx = L_lum_cmplx | L_rg_cmplx;

% No L-M response
L = out(:,4) < 100 | out(:,5) < 100;
L = out(:,3) > out(:,5)*100
data(out(L & L_cmplx,1)).filename
% No L+M response
L = out(:,2) == 0 & out(:,3) == 0;
data(out(L & L_cmplx,1)).filename

% Similar L-M and L+M
L = L_lum_cmplx & L_rg_cmplx & abs(log10(out(:,3)./out(:,5))) < 0.1;
data(out(L & L_cmplx,1)).filename
