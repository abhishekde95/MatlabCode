...
% QUEST PARAMETERS
q.muPrior = [];                 % best guesses for threshold (nColors x nSfs)
        ...
        switch stim.exptMethod
            ...
            case 3 %QUEST
                ...
                for c = 1:size(q.colorDirs, 1);
                    q.range{c} = 0.01:0.05:maxContrast;
                    % set up the priors
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function setupTrial(RFpos, flashLength, fpPos, fpSize, frameOffset, targSize, T1Dist, T2Dist, gaborTheta, gaborGamma, gaborSigma, gaborSpeed, nSigmas, lastTrialGood, lastTrialCorrect)
        ...
        %spatial location of the FP, wire frame, stimulus flash, and targets
        ...
        %make the stimulus movie ahead of time.
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function movie = makeMovie(X,Y);
        
        % zero this out here (and not in AllOff. Otherwise the number of
        % frames won't be droped with the trialParams.
        stim.flashFrameCounter = 0;
        
        % now go about making the stimulus movie
        ...
        switch fitMethod
            case 1; %gratings
                ...
                if gl.grating.prefsf %prevent Infs when prefsf = 0
                    ...
                    stim.gaborLambda = eps; %don't let the sfs = 0
                end
                stim.gaborTheta = gl.grating.preforient;                     % radians
                stim.gaborPhase = 0;
                stim.gaborSigma = 4;                                         % hard coding the sigma !!!
                ...
                if isfield(gl.grating, 'prefcolors')
                    prefcolors = reshape(gl.grating.prefcolors, 3, 2)';
                    prefSfs = gl.grating.prefsf;
                    load ('/REX slave programs/sCSF_Fits.mat');
                    for a = 1:2;
                        [val, idx] = ismember(sign(prefcolors(a,:)), sign(fp.colors), 'rows');
                        ...
                        %by 100;
                        stim.prefColors(1, ((a-1)*3)+1 : a*3) = 2.2 .* (1./sensitivity) .* fp.colors(idx, :) .* 100; %go to 2.2 times threshold
                    end
                end
                ...
                %intermediate color
                if (stim.prefColors(1:3) * stim.prefColors(4:6)') < 0;
                    stim.prefColors(1:3) = stim.prefColors(1:3) .* -1;
                end
                disp(stim.prefColors);
...
%HANDLE UDP COMMUNICATIONS
    function dealWithUdpMessages()
        ...
        msgSize = pnet(udpCom.sock, 'readpacket', 260, 'noblock');
        if ~msgSize
            return
        end
