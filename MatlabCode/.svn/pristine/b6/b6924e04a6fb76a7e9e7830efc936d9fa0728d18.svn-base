...
idx = [OCidx LMidx LUMidx hardtoclassifyidx];
resize_fact = 1;
...
for ii = 1:numel(idx)
    disp(ii);
    filename = Input_List{idx(ii)}; % acquiring the filename from the List
...
%     keyboard;
    ...
    % Subunits 
    subplot(num_rows,C,((ii-(kk-1)*num_rows)-1)*C+5); imagesc(Output_List{ii,7}); set(gca,'XTick',[],'YTick',[]); axis square;
    if mod(ii,num_rows) ==1 
        title('Subunits');
    end
...
subplot(221); plot(Gaborerror,DOGerror,'o','MarkerSize',6,'LineWidth',0.5,'MarkerFaceColor',[0 0 1],'MarkerEdgeColor',[1 1 1]); hold on; axis square;
...
subplot(224); histogram(deltaBIC,bins); axis square; xlabel('delta BIC'); ylabel('count'); set(gca,'TickDir','out');hold off;
