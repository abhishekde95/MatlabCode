...
int gl_ntrlspercond = 3;  /* Set to '0' to do trials until an ANOVA on the spike counts is significant */
...
setup(void)
{
     ...
     sprintf(buf, "InitDisplay(%d, %d, %s, %s, %d)",
                      GL_MONDIST, GL_SCREENWIDTH, GL_CALFILENAME, gl_fundfilename, gl_ccmode);
...
VLIST basic_vl[] =
{
	...
	{"ccmode (0=no++,1=yes++)", &(gl_ccmode), NP, NP, 0, ME_DEC},
	{NS},
};
...
begin	first:
   ...
   poststimdelay:
	...
	rewoff:
		do dio_off(REW)
		time 300							/* YES: added pause before extra reward state, and commented out to getnframes */
		to rewonextra
		/* to getnframes */				
	rewonextra:			
		do dio_on(REW)
