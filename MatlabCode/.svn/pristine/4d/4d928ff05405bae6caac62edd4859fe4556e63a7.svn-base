function [expt] = DTmocsUnpack_JPW(stro,defaultParams)
...
%grandZscores{1,2} = [];
for a = 1:nColors
    %figure(6); %open a new figure for each color direction
    ...
    if ispc
        slashChar = strfind(stro.sum.fileName, '\');
    elseif ismac
        slashChar = strfind(stro.sum.fileName, '/');
    else
        slashChar = strfind(stro.sum.fileName, '/'); %Added by JPW to avoid an error when running on the cluster
    end
    subplot(nFigRows,nSptFreqs,a)
    set(gcf, 'Name', sprintf('%s', stro.sum.fileName(slashChar(end)+1:dots(end)-1)));
    set(gcf,'NumberTitle', 'off');
    for j = 1:nSptFreqs;
        subplot(nFigRows, nSptFreqs, a);
        %set(gcf, 'Title', sprintf('[%.2f, %.2f, %.2f]', colorLabels(1), colorLabels(2), colorLabels(3)));
        title( sprintf('Color Directon: L:%.2f, M:%.2f, S:%.2f', colorLabels(1), colorLabels(2), colorLabels(3)));
        %set(gcf,'NumberTitle', 'off');
        %if ~isempty(cellFits)
        %    title(sprintf('sptFreq: %.2f cyc/deg \n \\alpha_{p}=%.3f, \\alpha_{n}=%.3f \n \\beta_{p}=%.3f, \\beta_{n}=%.3f', expt.sfs(j), monkFits.alpha(a,j), monkFits.beta(a,j)));
        %else
...
function monkFits = psychFun(norms, nTrialsByCntrst, a, b, nFigRows, nSptFreqs, monkFits,s)
    ...
    subplot(nFigRows, nSptFreqs, a);hold on;grid on;
    h1 = semilogx(norms{a,b}, monkFits.performance{a,b}, 'b.');
    %h3 = semilogx([0:dNorm:max(norms{a,b})], modelMLE+.01, 'r--');
    h2 = semilogx([0:dNorm:max(norms{a,b})], modelMLE, 'b--');
...
%             otherwise
%                 error('start type <%s> not yet supported', anlyStart)
%         end
...
%             otherwise
%                 error('end type <%s> not yet supported', anlyEnd)
%         end
