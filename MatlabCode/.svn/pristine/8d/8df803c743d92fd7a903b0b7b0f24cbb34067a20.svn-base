...
function DoFlip() % tidy this up to work with targ
    ...
    if(gl.syncingrexandmac)
        pnet(udpCom.sock, 'write', 'MACSYNC>> >>');
        pnet(udpCom.sock, 'writepacket', udpCom.rexip, udpCom.port);
        gl.syncingrexandmac = 0;
        return % Do not want to increment framecounter
    end
    if(gl.fp.on == 0 && gl.targ.on == 0 && gl.framecounter == 0)
        pnet(udpCom.sock, 'write', 'MACSTIMON>> >>');
        pnet(udpCom.sock, 'writepacket', udpCom.rexip, udpCom.port);
    end
    if (gl.framecountermax > 0 && gl.framecounter == gl.framecountermax)
...
function HideFPandShowTarg(nframesgap, x, y, targsize, fpr, fpg, fpb)
    global gl

    gl.fp.on = 0;
    gl.framecounter = 0;
    gl.framecountermax = nframesgap;
    gl.targ.x = x/10;
    gl.targ.y = y/10;
