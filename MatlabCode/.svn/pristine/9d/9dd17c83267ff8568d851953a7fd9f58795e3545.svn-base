% This code fits curves to psych and chron data for two k's, two B's, one A,
% and one Tnd for H and L coherences

function [th_fit10,err10] = fitwithcurves10(df,tg)

%% Set Variables

opts = optimset('MaxFunEvals',10.^6,'MaxIter',10.^6);
tg(3) = sqrt(max(max(df(1).meanrt),max(df(2).meanrt)));
tg(4) = min(min(df(1).meanrt),min(df(2).meanrt));

[th_fit10,err10]=fminsearch('calcparams10',tg(1:6),opts,df);
...
title('Psychometric Fit to High and Low Coherences (Different ks and Bs)')
...
title('Chronometric Fit to High and Low Coherences (Different ks and Bs)')
