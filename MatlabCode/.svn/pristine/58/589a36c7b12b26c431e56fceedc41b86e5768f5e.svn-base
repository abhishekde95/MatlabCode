function [err] = calcparams1(tg,df)
...
% High coherences
tTH1=(A./(kH.*cohH1)).*tanh(kH.*A.*cohH1)+Tnd;
tTH2=(A./(kH.*cohH2)).*tanh(kH.*A.*cohH2)+Tnd;

% Low coherences
tTL1=(A./(kL.*cohL1)).*tanh(kL.*A.*cohL1)+Tnd;
tTL2=(A./(kL.*cohL2)).*tanh(kL.*A.*cohL2)+Tnd;
