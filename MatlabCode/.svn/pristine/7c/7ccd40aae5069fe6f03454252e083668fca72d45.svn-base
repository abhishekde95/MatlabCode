...
NUMMODELS = 5; % 1-Crescent, 2-Gabor, 3-DoG, 4-Single Gaussian, 5-Single Gaussian with indepenent variances along x and y
SSE = cell(numcells,NUMMODELS); 
Deviation = cell(numcells,NUMMODELS);
Peraccuracy = cell(numcells,NUMMODELS); 
...
for ii = 1:numcells
    ...
    SSE_Crescent = []; SSE_Gabor = []; SSE_DOG = []; SSE_SG = []; SSE_S2G = [];
    Peraccuracy_Crescent = []; Peraccuracy_Gabor = []; Peraccuracy_DOG = []; Peraccuracy_SG = []; Peraccuracy_S2G = [];
    Deviation_Crescent = []; Deviation_Gabor = []; Deviation_DOG = []; Deviation_SG = []; Deviation_S2G = [];
...
%         subplot(c.NumTestSets,5,5*jj); image(255*(fittedDOG./(2*max(abs(fittedDOG(:))))+.5)); axis image; set(gca,'XTick',[],'YTick',[]); colormap(gray(255));
    ...
    SSE{ii,1} = SSE_Crescent; SSE{ii,2} = SSE_Gabor; SSE{ii,3} = SSE_DOG; SSE{ii,4} = SSE_SG; SSE{ii,5} = SSE_S2G;
    Peraccuracy{ii,1} = Peraccuracy_Crescent; Peraccuracy{ii,2} = Peraccuracy_Gabor; Peraccuracy{ii,3} = Peraccuracy_DOG; Peraccuracy{ii,4} = Peraccuracy_SG; Peraccuracy{ii,5} = Peraccuracy_S2G;
    Deviation{ii,1} = Deviation_Crescent; Deviation{ii,2} = Deviation_Gabor; Deviation{ii,3} = Deviation_DOG; Deviation{ii,4} = Deviation_SG; Deviation{ii,5} = Deviation_S2G;
