function [s,k,h,dc,prs,kbasis,hbasis] = fit_glm_AD(x,y,dt,nkt,kbasprs,ihbasprs,prs,softRect,plotFlag,maxIter,tolFun,L2pen,spatiochromatic_guess, temporal_guess)
...
if ~exist('prs','var') || isempty(prs)
    N_spatialparams = numel(spatiochromatic_guess); % Number of spatial parameters 
    prs = zeros(N_spatialparams+nkbasis+nhbasis+1,1); % initialize spatial and temporal parameters
    prs(1:N_spatialparams) = spatiochromatic_guess; % intial guess of spatial parameters 
end
...
% x1 = x*spatiochromatic_guess; % Calculating the spatiochromatic drive by projecting the spatiochromatic stimuli onto the

for knum = 1:nkbasis
    xconvki = [xconvki sameconv(x,kbasis(:,knum))];
end
...
if softRect
    ...
    NL = @exp;
    fneglogli = @(prs) negloglike_glm_basis_AD(prs,NL,xconvki,yconvhi,y,1,refreshRate,L2pen,N_spatialparams); % using this for the time being
end
...
s = prs(1:N_spatialparams); % spatial filters
k = kbasis*prs(N_spatialparams+1:N_spatialparams+nkbasis); % k basis functions weighted by given parameters
h = hbasis*prs(N_spatialparams+nkbasis+1:end-1); % k basis functions weighted by given parameters
