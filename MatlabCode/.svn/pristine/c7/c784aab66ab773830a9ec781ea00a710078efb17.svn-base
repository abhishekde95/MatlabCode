...
long gl_screenwidthinpix = 0;         /* Number of pixels across the width of the monitor */
...
const char NUDGE_REQ[] = "nudgerequest();";
...
checkmsg()
{
    ...
    if (messageAvailable) {
        ...
        if (!strcmp(message.header, "MACSTIMON")) {
            ...
            gl_screenwidthinpix  = (int) hex2long(message.contents, NULL);
            gl_mac_ready = 1;
        } else if (!strcmp(message.header, NUDGE_REQ)) {
            hex2double(message.contents, (void *)&doublearray);
            doublearray[0] = doublearray[0]*2*gl_npixperstix/gl_screenwidthinpix*GL_SCREENWIDTH; // nudge in cm
            doublearray[1] = doublearray[1]*2*gl_npixperstix/gl_screenwidthinpix*GL_SCREENWIDTH; // nudge in cm
            gl_rfx = gl_rfx+my_round((float)atan2(doublearray[0], (double) GL_MONDIST)*180/M_PI*10);
            gl_rfy = gl_rfy+my_round((float)atan2(doublearray[1], (double) GL_MONDIST)*180/M_PI*10);
