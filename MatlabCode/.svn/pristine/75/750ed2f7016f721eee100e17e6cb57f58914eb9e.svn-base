...
%% Variability in cell response...

[b,bint,r,rint,stats] = regress(par.varnspikes,par.meannspikes);
x = 0:.1:max(par.meannspikes)*1.1;
y = b * x;
slope(loop) = b;

figure(fig); hold on; grid on; fig=fig+1;
plot(par.meannspikes,par.varnspikes,'*k')
plot(x,y,'--')
...
%% Plot histogram of all b values
