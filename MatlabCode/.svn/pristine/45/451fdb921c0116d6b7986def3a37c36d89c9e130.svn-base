...
[xN yN] = meshgrid(X,X);
ZZ = RFModel(yN,xN,nPts1D_MG_NC,DVAperStix,0);
...
for r = 1:nRnds
    ...
    if r == 1
        ...
        sqIdx = find(cat(1,square.test)==1);
        for sq = 1:numel(sqIdx)
            
            % Use only corners of the squares as the initial grid.
            upperLeft = [square(sqIdx(sq)).r(1,1) square(sqIdx(sq)).c(1,1)];
            lowerLeft = [square(sqIdx(sq)).r(end,1) square(sqIdx(sq)).c(end,1)];
            upperRight = [square(sqIdx(sq)).r(1,end) square(sqIdx(sq)).c(1,end)];
            lowerRight = [square(sqIdx(sq)).r(end,end) square(sqIdx(sq)).c(end,end)];
        ...
        responses(ptsInRnd_MG_I) = mean(tempResp,2);
        ...
        ptsInRnd_MG_I = sub2ind(size(masterGrid_C_c),...
            abs(ptsInRnd_MG_C_r-max(masterGrid1D_C)-1),...
