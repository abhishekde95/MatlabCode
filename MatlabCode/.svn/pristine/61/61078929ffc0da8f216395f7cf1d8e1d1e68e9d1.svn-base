...
function [out,fittedCrescent,R_square,Pearson_R] = Crescentfit_AD(im,DOGfitparams)
...
%     keyboard;
    ...
    numerator = fval; %min([sum((fittedCrescent(:)+im(:)).^2) sum((fittedCrescent(:)-im(:)).^2)]);
    denominator = sum(im(:).^2.*(im(:)-mean(im(:))).^2);
    R_square = 1-(numerator/denominator);
    
    Pearson_R = acos(abs(corr(fittedCrescent(:),im(:))))*180/pi;
