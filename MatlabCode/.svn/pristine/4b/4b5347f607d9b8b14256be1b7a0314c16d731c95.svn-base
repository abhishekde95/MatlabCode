function [gab, cones, mon, idlob, params] = DTcones_gh(params,saveflag)
%
%  DTcones_gh(params)
%
...
% Created by CAH using cone physiology data (and fits) from Juan Angueyra
    ...
    cones = initConeMosaic(gab, mon, eyes, params);
...
function [gab, params] = initGabor(params)
    ...
    switch lower(params.runType)
        ...
        case 'isosamp'
            ...
            else % 4 SD is arbitrary.
                gab.sd = params.gab.sd*10; % Interpreting params.gab.sd as the sd of the Gaussian field of cones we're integrating over (could be product of RF and Gabor)
                gab.nSd = 2;
            end
            ...
            % 8/9/19 passing in poralIntegrationLims
            ...
            % 8/9/19 Reverting all changes below
            % Implementing subideal observer by truncating "wt_spaceTime"
            % in obsMethod_filteredWtFxn instead of truncating the stimulus
            % (which leads to bad edge effects as cone responses increase
            % slowly to the abrupt stimulus onset.
            
%             if isfield(params,'timewindoffset') && ~isempty(params.timewindoffset)
%                 timewindoffset = params.timewindoffset;
%             else
%                 timewindoffset = [0 unique(params.stro.trial(:,flashtime_idx))/1000];
%             end
%             
%             % GDLH 7/23/18: moving declaration of flashTimeProf from
%             % makeGaborMovie to here and making it part of the "gab"
%             % structure. This will allow us to modify the temporal profile
%             % of the stimulus to simulate non-ideal observers.
%             
%             % flashNumFrames is the duration of the full stimulus
...
%             % full stimulus).
%             flashNumFrames = ceil(params.monCalFile.frameRate * unique(params.stro.trial(:,flashtime_idx))/1000);
...
%             flashTimeVector = [0:flashNumFrames-1]/params.monCalFile.frameRate;
%             movieNumFrames = ceil((timewindoffset(2)-timewindoffset(1))*params.monCalFile.frameRate); % This is the length of the entire movie
%             movieTimeVector = [0:movieNumFrames-1]/params.monCalFile.frameRate+timewindoffset(1);
%             
%             gab.flashTimeProf = zeros(1,movieNumFrames);
%             startFrameMovie = find((movieTimeVector-flashTimeVector(1)).^2 == min((movieTimeVector-flashTimeVector(1)).^2));
%             startFrameFlash = find((movieTimeVector(1)-flashTimeVector).^2 == min((movieTimeVector(1)-flashTimeVector).^2));
%             endFrameMovie = min(flashNumFrames-startFrameFlash+startFrameMovie,movieNumFrames);
%             endFrameFlash = min(movieNumFrames-startFrameMovie+startFrameFlash,flashNumFrames);
%             gab.flashTimeProf(startFrameMovie:endFrameMovie) = flashTimeProf(startFrameFlash:endFrameFlash);
%             %plot(movieTimeVector,gab.flashTimeProf);
%             %drawnow;
%             gab.length = timewindoffset(2)-timewindoffset(1);
            
            % GDLH 7/23/18 end of changes
...
%     %-------------------------------------   GDLH 
     sacamp_pix = mon.pixperdeg*params.sacamp_deg;
     sacdur_frames = params.sacdur_s*mon.frameRate;
%     % Just vertical component (in pixels vs frames)

     flashNumFrames = ceil(mon.frameRate * gab.length);
     EM = zeros(flashNumFrames,1); % Start
     sacframes = round(flashNumFrames/2):round(flashNumFrames/2+sacdur_frames);
     EM(sacframes) = linspace(0,sacamp_pix,length(sacframes)); % during saccade
     EM(sacframes(end):end) = sacamp_pix; % End
     EM = EM-mean(EM); % Centering
%     %-------------------------------------    

    % Define the constants
    %nPix = round((gab.nSd*2) .* (gab.sd./10) .* mon.pixperdeg)+range(EM); % GDLH
    ...
    for t = 1:flashNumFrames
        ...
        %gabor = exp(-(xprime.^2 + gab.gamma.^2 .* (yprime-EM(t)).^2) ./ (2.* sigmaInPix.^2)) .* cos(2 .* pi .* (yprime-EM(t)) ./ lambdaInPix + gaborPhase); % GDLH
...
% GH: This needs to be changed. Assumes a specific size cone mosaic. Can't
% deal with eye movements
function cones = initConeMosaic(gab, mon, eyes, params)
    

    %sacamp_pix = abs(mon.pixperdeg*params.sacamp_deg);

    % start by calculating the visual angle subtended by a single pixel
%     gaborWidth_pix = round((gab.nSd*2) .* (gab.sd./10) .* mon.pixperdeg); %should be ~125 pix for a 2.4 dva stimulus
    
    gaborWidth_pix = round((gab.nSd*2) .* (gab.sd./10) .* mon.pixperdeg); %+sacamp_pix; % GH added 5/3/15

    
    if rem(gaborWidth_pix,2)
        gaborWidth_pix = gaborWidth_pix+1; % make this an even number for consistency with the Gabor movie
    end
   % gaborWidth_dva = (gab.sd/10) .* (gab.nSd .* 2); % GH: This probably needs to change
    gaborWidth_dva = gaborWidth_pix./mon.pixperdeg; % GH: Added 5/3/15. Is this OK?
...
function [idlob, cones] = obsMethod_filteredWtFxn(command, idlob, gab, cones, mon, clr, cnt, trl, params)
    ...
    switch lower(command)    
        ...
        case 'analytic_solution'
            ...
            % directions) so just calculate it on the first trial and then
            % carry it forward for the other contrasts
            % GDLH Not true anymore. Commenting out skipping.
            %if cnt == 1;
