...
% -----------------------------------------------
% GDLH: adding some stuff to add in a saccade
% This parameterization of the eye movement is pretty
% lame, but I don't know how else to do it because we need
% to know how many frames the stimulus presentation lasts.
% The duration of the trial is set just below in dtnt.length,
% but the framerate of the monitor isn't set until initMonitorCalibration.
% -----------------------------------------------
% Creating an eye movement trace with a microsaccade in the middle.
% If these numbers are correct, a 0.2° microsaccade occurs over the
% duration of a single frame at 75 Hz.
params.sacamp_deg = 0.2; % Positive numbers mean saccade is in the direction of motion
params.sacdur_s = 0.013; % Is this reasonable?
%------------------------------------------------

% make some notes...
params.notes = sprintf('Trying the eye movement patch. Amplitude = %g, Duration = %g\n', ...
    params.sacamp_deg, params.sacdur_s);
