function [err] = calcparams5(tg,df)
...
% High coherences
tTH1=(A./(k.*(cohH1-Cb))).*tanh(k.*A.*(cohH1-Cb))+Tnd;
tTH2=(A./(k.*(cohH2-Cb))).*tanh(k.*A.*(cohH2-Cb))+Tnd;

% Low coherences
tTL1=(A./(k.*(cohL1-Cb))).*tanh(k.*A.*(cohL1-Cb))+Tnd;
tTL2=(A./(k.*(cohL2-Cb))).*tanh(k.*A.*(cohL2-Cb))+Tnd;

% % Psychometric function
% High coherences
PcH1=1./(1+exp(-2*k*A*(cohH1-Cb)))';
PcH2=1./(1+exp(-2*k*A*(cohH2-Cb)))';

% Low coherences
PcL1=1./(1+exp(-2*k*A*(cohL1-Cb)))';
PcL2=1./(1+exp(-2*k*A*(cohL2-Cb)))';
