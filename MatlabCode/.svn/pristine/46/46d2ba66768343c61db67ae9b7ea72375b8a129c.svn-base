...
while ~allDone
    ...
    %bail on this loop if the trial was bad or if there's no header, or if
    %the experimental isn't MoCS.
    ...
    elseif d.eParams.expt_meth ~= 1;
        dControls('newTrial')
        continue %bail if the exptMeth is not MoCS
    end
    
    %deal with fixation (grating) trials. If you make it to these lines of
    %code than I'ts a good trial. If it's a 'good' trial AND a fixation
    %trial, than reset the grating trial type code. 
    gratingTrial = dat2num(d.tParams.codes, C.TRIALTYPECD, C.VALOFFSET, 'int');
    if gratingTrial{1} == 1
        enforceGrating([], [], 'reset');
        dControls('newTrial');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function setupFig(maxNumNeurons)
        ...
        nextTrialGrating = 0; % a flag I'll put in the user data for the grating push button.
        d.figs.ui.gratingButton = uicontrol('style', 'pushbutton', 'string', 'Grating', 'callback', {@enforceGrating, 'set'}, 'position', [29, figHeight.*0.06, 60, 25], 'userdata', nextTrialGrating);
        drawnow
    end
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function enforceGrating(h, ev, process)
        ...
        nextTrialGrating = get(d.figs.ui.gratingButton, 'userdata');
        switch lower(process)
            case 'set'
                nextTrialGrating = 1;
                %change the color here.
            case 'query'
                sendToRex(udpCom, nextTrialGrating, 'integer', 'enforceGrating([], [], ''query'');');
            case 'reset'
                nextTrialGrating = 0; %re-initialize the flag when a complete grating trial has occured
                %change the color here
        end
        set(d.figs.ui.gratingButton, 'userdata', nextTrialGrating);
        %enforce the color change here.
    end
