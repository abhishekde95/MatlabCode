...
set(gcf,'units','normalized','pos',[.15 .15 .7 .7],'NumberTitle','off',...
    'Name','Radially Symmetric Distribution');
...
conpanel.uicontrols.maxcc = uicontrol('parent',modelfig.conpanel,'units','normalized',...
    'style','edit','pos',[.1 .4 .35 .08],'string',.09,'fontsize',10);
conpanel.labels.maxcc = uicontrol('parent',modelfig.conpanel,'units','normalized',...
    'pos',[.1 .48 .35 .07],'style','text','HorizontalAlignment','center',...
    'string','Max Contrast','fontsize',10);
...
gl.maxcc = str2double(get(conpanel.uicontrols.maxcc,'string'));
...
for rot = 1:numel(gl.allAngs)
    ...
    for sampn = 1:gl.nSamps
        ...
        disp('Estimating preferred direction by RWA...')
...
data.RWA.means = gl.RWAfitmean;
data.RWA.std = gl.RWAfitstd;
data.LL.means = gl.LLfitmean;
data.LL.std = gl.LLfitstd; 
if ismac
    save('/Users/jpatrickweller/Dropbox/VisionResearchPaper/Fig3aData(RS)','data');
else ispc
    save('C:\Users\jpweller\Dropbox\VisionResearchPaper\Fig3aData(RS)','data');
end

% Say goodbye
disp('Fig 3a Modeling Completed.')
...
for n = 1:numel(whichangs)
    ...
    gl.params(end) = ang;
    scalefac = 20;
    [uniquepts,~,b] = unique([gl.Lcc gl.Mcc],'rows');

    % Responses to stimuli
    gl.nsp = ComputeNakaRushtonJPW(gl.params,[gl.Lcc gl.Mcc],'surface7');
    
    axes(surfpanel.axes(n)); cla; grid on; hold on; 
    for i = 1:size(uniquepts,1)
        L = i == b;
        meannsp = mean(gl.nsp(L));
        h = plot(uniquepts(i,1),uniquepts(i,2),'ko');
        set(h,'MarkerFaceColor','black','MarkerSize',(((meannsp-min(gl.nsp))/max(gl.nsp))+.2).*scalefac,'MarkerEdgeColor','white');
    end
    ...
    angub = ang + gl.RWAfitstd(whichangs(n))/180*pi;
    anglb = ang - gl.RWAfitstd(whichangs(n))/180*pi;
    [x1,y1] = pol2cart(angub,max(gl.Lcc)*2);
    [x2,y2] = pol2cart(anglb,max(gl.Lcc)*2);
    h1 = patch([0 x1 x2],[0 y1 y2],[.5 .5 .8],'r');
    alpha(.4)
    angub = ang + gl.LLfitstd(whichangs(n))/180*pi;
    anglb = ang - gl.LLfitstd(whichangs(n))/180*pi;
    [x1,y1] = pol2cart(angub,max(gl.Lcc)*2);
    [x2,y2] = pol2cart(anglb,max(gl.Lcc)*2);
    h1 = patch([0 x1 x2],[0 y1 y2],[.5 .5 .8],'b');
    alpha(.4)
    xlim([min(gl.Lcc)*1.1 max(gl.Lcc)*1.1]);
    ylim([min(gl.Mcc)*1.1 max(gl.Mcc)*1.1]);
...
plot([min(angs) max(angs)],[0 0],'k-') 
h = shadedErrorBar(angs,gl.RWAfitmean',gl.RWAfitstd,'r-');
set(h.patch,'FaceAlpha',.4)
h = shadedErrorBar(angs,gl.LLfitmean',gl.LLfitstd,'b-');
set(h.patch,'FaceAlpha',.4)
xlim([min(angs) max(angs)])
maxy = max(abs(cat(1,(gl.RWAfitstd+gl.RWAfitmean),(gl.LLfitstd+gl.LLfitmean))));
...
gl.Lcc = tempLcc .* gl.maxcc;
gl.Mcc = tempMcc .* gl.maxcc;
