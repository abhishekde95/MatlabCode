...
set(gcf,'units','normalized','pos',[.15 .15 .7 .7],'NumberTitle','off',...
    'Name','Linearly Transformed RS Distribution');
...
conpanel.labels.scale = uicontrol('parent',modelfig.conpanel,'units','normalized',...
    ...
    'string','Linear Scale','fontsize',10);
...
if ismac
    save('/Users/jpatrickweller/Dropbox/VisionResearchPaper/Fig2bData(transRS)','data');
else ispc
    save('C:\Users\jpweller\Dropbox\VisionResearchPaper\Fig2bData(transRS)','data');
end

% Say goodbye
disp('Data saved to Dropbox.')
disp('Fig 2b Modeling Completed.')
...
for n = 1:numel(whichangs)
    ...
    for i = 1:size(uniquepts,1)
        ...
        set(h,'MarkerFaceColor','none','MarkerSize',(((nsp(i)-min(nsp))/max(nsp))+.2).*scalefac,'MarkerEdgeColor','black');
    end
...
% Setting sigma according to an ellipse
% colmaxcc = str2double(conpanel.uicontrols.maxcc.String);
% linscale = str2double(conpanel.uicontrols.scale.String);
% lummaxcc = colmaxcc * linscale;
% scale = lummaxcc*colmaxcc./sqrt((colmaxcc.*cos(ang-pi/4)).^2 ...
