...
subplot(4,1,1)
plot(Stim.Wavelength,Stim.Spectrum,'o-');
title('Stimulus Spectrum')
xlabel('Wavelength (nm)')
ylabel('Relative Intensity')
subplot(4,1,2)
plot(Stim.Wavelength,StimPowerSpectra,'o-')
title('Power Spectrum of the Stimulus')
xlabel('Wavelength (nm)')
ylabel('Watts (J/s)')
subplot(4,1,3)
plot(Stim.Wavelength,PhotonSpectra,'o-')
...
plot(SpaceAxis,real(F),'b-')
plot(SpaceAxis,imag(F),'c--')
...
%% Part VII: Impulse Response Function

% Choose width of the bar (Movshon chose bar width as between 1/8 and 1/16
% the size of the receptive field)
nbars = 12; % to parallel the Movshon experiments
barwidth = RFDiameter/nbars; %in mm

binmarkers = linspace(-max(Ganglion.Boarders),max(Ganglion.Boarders),2*nbars+1);
binboarders = binmarkers(1:2:end);
bincenters = binmarkers(2:2:end);
for n = 1:nbars
    nLC(n) = sum(binboarders(n) <= Ganglion.Center.Coordinates.LCones(:,1) &...
        Ganglion.Center.Coordinates.LCones(:,1) <= binboarders(n+1));
    nLS(n) = sum(binboarders(n) <= Ganglion.Surround.Coordinates.LCones(:,1) &...
        Ganglion.Surround.Coordinates.LCones(:,1) <= binboarders(n+1));
    nMC(n) = sum(binboarders(n) <= Ganglion.Center.Coordinates.MCones(:,1) &...
        Ganglion.Center.Coordinates.MCones(:,1) <= binboarders(n+1));
    nMS(n) = sum(binboarders(n) <= Ganglion.Surround.Coordinates.MCones(:,1) &...
        Ganglion.Surround.Coordinates.MCones(:,1) <= binboarders(n+1));
end

Lonpbin = nLC' * LCexcite;
Loffpbin = nLS' * LSexcite;
Monpbin = nMC' * MCexcite;
Moffpbin = nMS' * MCsuppress;

respbins = Lonpbin + Loffpbin + Monpbin + Moffpbin;

figure(9);clf;hold on;
plot(bincenters,respbins,'o-')


%% Part ?: Drift Stimulus Across Receptive Field (Under Construction - not pertinant
% to Pbio545, but eventually may be useful.)
%
% %testwidths = .02
% Stim.DriftRate = 1;
% 
% %for z = 1:numel(testwidths)
% 
% Stim.Width = .02 ; % in mm on retina
% Stim.dtdx = 1/(Stim.Width * Stim.DriftRate / Stim.SamplingRate); %mm/sample
% 
% 
% % Calculate delays for L Cones
% Lptsdelay = round((Cones.Coordinates.LCones(:,1) - min(Cones.Coordinates.LCones(:,1)))* Stim.dtdx);
% mindelayL = min(Cones.Coordinates.LCones(:,1));
% LCptsdelay = round((Ganglion.Center.Coordinates.LCones(:,1) - mindelayL)* Stim.dtdx);
% LSptsdelay = round((Ganglion.Surround.Coordinates.LCones(:,1) - mindelayL)* Stim.dtdx);
% 
% % Calculate delays for M Cones
% Mptsdelay = round((Cones.Coordinates.MCones(:,1) - min(Cones.Coordinates.MCones(:,1)))* Stim.dtdx);
% mindelayM = min(Cones.Coordinates.MCones(:,1));
% MCptsdelay = round((Ganglion.Center.Coordinates.MCones(:,1) - mindelayM)* Stim.dtdx);
% MSptsdelay = round((Ganglion.Surround.Coordinates.MCones(:,1) - mindelayM)* Stim.dtdx);
% 
% % Redefine time axis
% newtimepts = numel(TimeAxis) + max(Lptsdelay);
% nTimeAxis = (1:newtimepts)./Stim.SamplingRate;
% 
% % Characterize Responses (on and off)
% onresponseL = (-1*(Cones.Responses(1,:) - Cones.Responses(1,1)))+Cones.Responses(1,1);
% onresponseM = (-1*(Cones.Responses(2,:) - Cones.Responses(2,1)))+Cones.Responses(2,1);
% onresponseS = (-1*(Cones.Responses(3,:) - Cones.Responses(3,1)))+Cones.Responses(3,1);
% 
% offresponseL = Cones.Responses(1,:);
% offresponseM = Cones.Responses(2,:);
% offresponseS = Cones.Responses(3,:);
% 
% % Fill in missing pieces of newtimeaxis with baseline activity
% Ganglion.Center.Responses.LCones = onresponseL(1) .* ones(numel(LCptsdelay),newtimepts);
% Ganglion.Surround.Responses.LCones = offresponseL(1) .* ones(numel(LSptsdelay),newtimepts);
% 
% Ganglion.Center.Responses.MCones = onresponseM(1) .* ones(numel(MCptsdelay),newtimepts);
% Ganglion.Surround.Responses.MCones = offresponseM(1) .* ones(numel(MCptsdelay),newtimepts);
% 
% 
% % Response Delay for L Cones (Center)
% for n = 1:numel(LCptsdelay)
%     Ganglion.Center.Responses.LCones(n,LCptsdelay(n)+1:LCptsdelay(n)+numel(onresponseL)) = onresponseL;
% end
% % Response Delay for L Cones (Surround)
% for n = 1:numel(LSptsdelay)
%     Ganglion.Surround.Responses.LCones(n,LSptsdelay(n)+1:LSptsdelay(n)+numel(offresponseL)) = offresponseL;
% end
% % Response Delay for M Cones (Center)
% for n = 1:numel(MCptsdelay)
%     Ganglion.Center.Responses.MCones(n,MCptsdelay(n)+1:MCptsdelay(n)+numel(onresponseM)) = onresponseM;
% end
% % Response Delay for L Cones (Surround)
% for n = 1:numel(MSptsdelay)
%     Ganglion.Surround.Responses.MCones(n,MSptsdelay(n)+1:MSptsdelay(n)+numel(offresponseM)) = offresponseM;
% end
% 
% % Sum responses (average, to avoid huge sums)
% allresp = cat(1,Ganglion.Center.Responses.LCones,Ganglion.Surround.Responses.LCones,...
%     Ganglion.Center.Responses.MCones,Ganglion.Surround.Responses.MCones);
% meanLresp = mean([Ganglion.Center.Responses.LCones;Ganglion.Surround.Responses.LCones]);
% meanMresp = mean([Ganglion.Center.Responses.MCones;Ganglion.Surround.Responses.MCones]);
% meanresp = mean(allresp);
% 
% figure(7); clf; hold on;
% plot(nTimeAxis,Ganglion.Center.Responses.LCones,'--r')
% plot(nTimeAxis,Ganglion.Surround.Responses.LCones,'--r')
% plot(nTimeAxis,Ganglion.Center.Responses.MCones,'--g')
% plot(nTimeAxis,Ganglion.Surround.Responses.MCones,'--g')
% plot(nTimeAxis,meanLresp,'r-')
% plot(nTimeAxis,meanMresp,'g-')
% plot(nTimeAxis,meanresp,'k')
% axis tight
% xlabel('Time (sec)')
