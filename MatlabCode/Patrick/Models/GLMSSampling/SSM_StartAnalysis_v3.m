...
function SSM_StartAnalysis_v3(~,~)
clear all
close all
global gl surfpanel fitspanel
...
gl.nPres = 1;
gl.nSamps = 1;
gl.allAngs = linspace(-pi/2,0,5);



% Preallocate Space
% PKA Surfpanel
tempsamp = struct('Lcc',[],'Mcc',[],'responses',[]);
samp(1:gl.nSamps) = tempsamp;
angle(1:numel(gl.allAngs)) = struct('samp',samp);
nrnds(1:3) = struct('angle',angle);
surfpanel = struct('nrnds',nrnds);
clear samp angle nrnds

% PKA Fitspanel
tempsamp = struct('testedangs',[],'GOF',[],'fitparams',[],'bestIdx',[],'bestAng',[],'error',[]);
samp(1:gl.nSamps) = tempsamp;
angle(1:numel(gl.allAngs)) = struct('samp',samp);
nrnds(1:3) = struct('angle',angle);
fitspanel = struct('nrnds',nrnds);
clear samp angle nrnds
...
gl.surfpanel = surfpanel;
gl.fitspanel = fitspanel;

temp1 = cat(1,fitspanel.nrnds(1).angle.samp);
gl.rnd1fits = cat(1,temp1.bestAng)-pi;
temp2 = cat(1,fitspanel.nrnds(2).angle.samp);
gl.rnd2fits = cat(1,temp2.bestAng)-pi;
temp3 = cat(1,fitspanel.nrnds(3).angle.samp);
gl.rnd3fits = cat(1,temp3.bestAng)-pi;
save elongated2_gl6 gl

figure(40); hold on;
plot(gl.allAngs/pi*180,gl.allAngs/pi*180,'k-')
plot(gl.allAngs/pi*180,gl.rnd1fits/pi*180,'r*')
plot(gl.allAngs/pi*180,gl.rnd2fits/pi*180,'g*')
plot(gl.allAngs/pi*180,gl.rnd3fits/pi*180,'b*')
...
scale = xmaxcc*ymaxcc./sqrt((ymaxcc.*cos(thetas)).^2+(xmaxcc.*sin(thetas)).^2);
surfpanel.nrnds(gl.currentRnd).angle(gl.currentAng).samp(gl.currentSamp).Lcc = tempLcc .* scale;
surfpanel.nrnds(gl.currentRnd).angle(gl.currentAng).samp(gl.currentSamp).Mcc = tempMcc .* scale;

% making some adjustments for figure creation
rotMat = [cos(pi/4) sin(pi/4); -sin(pi/4) cos(pi/4)];
tempPts = rotMat * [tempLcc tempMcc]';
tempLcc = tempPts(1,:)';
tempMcc = tempPts(2,:)';
plot(tempLcc.*scale,tempMcc.*scale,'ko')
hold on;

a = linspace(0,xmaxcc,5); %horizontal radius
b = linspace(0,ymaxcc,5); %vertical radius
