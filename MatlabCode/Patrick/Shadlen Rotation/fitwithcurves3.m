% This code fits curves to psych and chron data for two B's, one A, one k, and one Tnd, 
% for H and L coherences

function [th_fit3,err3] = fitwithcurves3(df,tg)
...
[th_fit3,err3]=fminsearch('calcparams3',tg(1:5),opts,df);
...
xax=-.4:.001:.4;
p_pred_psych2H=(1./(1+exp(-2*k*A*(xax-CbH))));
p_pred_psych2L=(1./(1+exp(-2*k*A*(xax-CbL))));
p_pred_chron2H=A./(k.*(xax-CbH)).*tanh(k.*A.*(xax-CbH))+Tnd;
p_pred_chron2L=A./(k.*(xax-CbL)).*tanh(k.*A.*(xax-CbL))+Tnd;
...
title('Psychometric Fit to High and Low Coherences (Different Bs)')
...
title('Chronometric Fit to High and Low Coherences (Different Bs)')
