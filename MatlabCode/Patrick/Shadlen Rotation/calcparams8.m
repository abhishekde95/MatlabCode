function [err] = calcparams8(tg,df)
...
% High coherences
tTH1=(AH./(kH.*cohH1)).*tanh(kH.*AH.*cohH1)+Tnd;
tTH2=(AH./(kH.*cohH2)).*tanh(kH.*AH.*cohH2)+Tnd;

% Low coherences
tTL1=(AL./(kL.*cohL1)).*tanh(kL.*AL.*cohL1)+Tnd;
tTL2=(AL./(kL.*cohL2)).*tanh(kL.*AL.*cohL2)+Tnd;

% % Psychometric function
% High coherences
PcH1=(1./(1+exp(-2*kH*AH*cohH1)))';
PcH2=(1./(1+exp(-2*kH*AH*cohH2)))';

% Low coherences
PcL1=(1./(1+exp(-2*kL*AL*cohL1)))';
PcL2=(1./(1+exp(-2*kL*AL*cohL2)))';
