function [err] = calcparams4(tg,df)
...
% High coherences
tTH1=(AH./(k.*(cohH1-CbH))).*tanh(k.*AH.*(cohH1-CbH))+Tnd;
tTH2=(AH./(k.*(cohH2-CbH))).*tanh(k.*AH.*(cohH2-CbH))+Tnd;

% Low coherences
tTL1=(AL./(k.*(cohL1-CbL))).*tanh(k.*AL.*(cohL1-CbL))+Tnd;
tTL2=(AL./(k.*(cohL2-CbL))).*tanh(k.*AL.*(cohL2-CbL))+Tnd;

% % Psychometric function
% High coherences
PcH1=1./(1+exp(-2*k*AH*(cohH1-CbH)))';
PcH2=1./(1+exp(-2*k*AH*(cohH2-CbH)))';

% Low coherences
PcL1=1./(1+exp(-2*k*AL*(cohL1-CbL)))';
PcL2=1./(1+exp(-2*k*AL*(cohL2-CbL)))';
