function [err] = calcparams3(tg,df)
...
% High coherences
tTH1=(A./(k.*(cohH1-CbH))).*tanh(k.*A.*(cohH1-CbH))+Tnd;
tTH2=(A./(k.*(cohH2-CbH))).*tanh(k.*A.*(cohH2-CbH))+Tnd;

% Low coherences
tTL1=(A./(k.*(cohL1-CbL))).*tanh(k.*A.*(cohL1-CbL))+Tnd;
tTL2=(A./(k.*(cohL2-CbL))).*tanh(k.*A.*(cohL2-CbL))+Tnd;

% % Psychometric function
% High coherences
PcH1=1./(1+exp(-2*k*A*(cohH1-CbH)))';
PcH2=1./(1+exp(-2*k*A*(cohH2-CbH)))';

% Low coherences
PcL1=1./(1+exp(-2*k*A*(cohL1-CbL)))';
PcL2=1./(1+exp(-2*k*A*(cohL2-CbL)))';
