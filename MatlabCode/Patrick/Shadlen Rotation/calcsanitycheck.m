function [err] = calcsanitycheck(tg,df_sanity)
...
tTL = (A./(kL.*cohL-(B/(2*A)))).*tanh(kL.*A.*cohL-(B/2))+Tnd;
tTH = (A./(kH.*cohH-(B/(2*A)))).*tanh(kH.*A.*cohH-(B/2))+Tnd;

% Psychometric function
PcL = 1./(1+exp(-2*kL*A*cohL+B))';
PcH = 1./(1+exp(-2*kH*A*cohH+B))';
