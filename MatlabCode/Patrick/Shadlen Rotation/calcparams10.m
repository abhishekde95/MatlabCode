function [err] = calcparams10(tg,df)
...
% High coherences
tTH1=(A./(kH.*(cohH1-CbH))).*tanh(kH.*A.*(cohH1-CbH))+Tnd;
tTH2=(A./(kH.*(cohH2-CbH))).*tanh(kH.*A.*(cohH2-CbH))+Tnd;

% Low coherences
tTL1=(A./(kL.*(cohL1-CbL))).*tanh(kL.*A.*(cohL1-CbL))+Tnd;
tTL2=(A./(kL.*(cohL2-CbL))).*tanh(kL.*A.*(cohL2-CbL))+Tnd;

% % Psychometric function
% High coherences
PcH1=1./(1+exp(-2*kH*A*(cohH1-CbH)))';
PcH2=1./(1+exp(-2*kH*A*(cohH2-CbH)))';

% Low coherences
PcL1=1./(1+exp(-2*kL*A*(cohL1-CbL)))';
PcL2=1./(1+exp(-2*kL*A*(cohL2-CbL)))';
