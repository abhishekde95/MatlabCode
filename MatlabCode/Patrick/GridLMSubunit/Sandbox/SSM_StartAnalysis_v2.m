...
zvals = (tempLcc./xmaxcc).^2 + (tempMcc./ymaxcc).^2;
zidx = zvals<=1;
%scale = xmaxcc*ymaxcc./sqrt((ymaxcc.*cos(thetas)).^2+(xmaxcc.*sin(thetas)).^2);
surfpanel.nrnds(gl.currentRnd).angle(gl.currentAng).samp(gl.currentSamp).Lcc = tempLcc(zidx);
surfpanel.nrnds(gl.currentRnd).angle(gl.currentAng).samp(gl.currentSamp).Mcc = tempMcc(zidx);

plot3(tempLcc(zidx),tempMcc(zidx),zvals(zidx),'k*')
axis equal
grid on;
keyboard
% Save variables
