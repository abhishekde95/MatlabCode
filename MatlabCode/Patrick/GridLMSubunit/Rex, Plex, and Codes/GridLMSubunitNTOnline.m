        ...
        if ~abortflag
            ...
            if epoch == 1 
                ...
                elseif gl.reset == 1 %Reset button has been pressed
                    ...
                    set(conpanel.uicontrols.resetbutton,'Value',0,'BackgroundColor',[.8 .5 .5],...
                        'string','Reset STA')
...
global gl
    ...
    elseif gl.epoch == 2
        ...
        if any(gl.CurrentQueue(:))
            ...
            if gl.addGabors == 1
                sub1L = gl.gridX == gl.sub(1).gridX && gl.gridY == gl.sub(1).gridY;% ID sub1 stim
                gaborqueue = gl.CurrentQueue(sub1L); %Grab all remaining stim for sub 1
                gaborqueue(:,end) = 3; % Assign them all to be epoch 3
                cat(1,gl.CurrentQueue,gaborqueue); %Put them at the end of the stim queue
...
function initGUI()
    ...
    function lumonoff(a,~)
        ...
        %set(a.axes.conpanel,'UserData',conpanel)
        if strcmp(a.String,'Lum On')
            gl.templum = .001;
            a.BackgroundColor = [1 .5 .5];
            a.String = 'Lum Off';
        elseif strcmp(a.String,'Lum Off')
            gl.templum = gl.lumcc;
            a.BackgroundColor = [.5 1 .5];
            a.String = 'Lum On';
        else
    ...
    function colonoff(a,~)
        if strcmp(a.String,'Col On')
            gl.tempcol = .001;
            a.BackgroundColor = [1 .5 .5];
            a.String = 'Col Off';
        elseif strcmp(a.String,'Col Off')
            gl.tempcol = gl.colcc;
            a.BackgroundColor = [.5 1 .5];
            a.String = 'Col On';
        else
    ...
    function addGabors(a,~)
        ...
        if strcmp(a.String,'Add Gabors')
            gl.addGabors = 1;
            a.BackgroundColor = [.5 1 .5];
            a.String = 'Gabors Added';
            disp('Adding gabors to the stimulus set...')
        elseif strcmp(a.String,'Gabors Added')
            gl.addGabors = 0;
            a.BackgroundColor = [1 .5 .5];
            a.String = 'Add Gabors';
