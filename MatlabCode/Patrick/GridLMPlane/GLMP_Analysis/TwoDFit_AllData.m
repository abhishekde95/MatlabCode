...
%                   rotation.)   JPW
% 1/4/13        Modified. (Added 'usenorm' variable, which allows user to
%                   switch between fitting to original dataset or
%                   normalized dataset.)    JPW


function plat = TwoDFit_AllData(plat,usenorm)
...
for p = 1:numel(plat)
    ...
    for rot = 1:nrots
        ...
        options = optimset('MaxFunEvals',100,'MaxIter',100,'TolFun',10^-6,'TolX',10^-6,'Display','off','Algorithm','active-set');
        sigmaguess = 0.1;
        ...
        if rot == 1
            for i = 1:2
                ...
                elseif isempty(topfr)
                    ...
                    f(i,:) = fmincon('FitNakaRushtonFunJPW',params0,[],[],[],[],vlb,vub,[],options,projs,plat{p}.trial.nspikes(L),'asymmetric');
                end
            end
            ...
            % Identify Important lines, points, and angles in the Original Space (L, M, L+M, L-M)
            if usenorm == 1
        ...
        [f0,fval] = fmincon('FitNakaRushtonFunJPW',params0,[],[],[],[],vlb,vub,[],options,tempRotPts,plat{p}.trial.nspikes,'surface1');
        params(rot,:) = f0;
        GOF{rot,p} = -fval;
    ...
    surface = ComputeNakaRushtonJPW(params(bestIdx,:),[x(:) y(:)],'surface1');
...
%     plot3(currentYAxis(:,1),currentYAxis(:,2),[0;0],'k')
    ...
    plat{p}.TwoDFitAllData.Best.RotMat = rotMat;
