...
%  stimulus
%**************************************************************************
[plot_counter] = WNpixelplot(stro,plot_counter,mask_changes(:,1));
...
for mode = mode_steps
    ...
    for mask_span = trial_span 
        ...
        elseif (mod(mode,3) == 2)
            ...
            [plot_counter,nbins,nbins1] = compute_projection_val(plot_counter,mode,subunits,1,1,flag);
        elseif (mod(mode,3) == 0)
            % Calculate the projection values of the stimuli on the white noise basis vectors  
            [plot_counter,~,~] = compute_projection_val(plot_counter,mode,subunits,nbins,nbins1,flag);
        end
    end
end
