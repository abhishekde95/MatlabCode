% A new script for doing cross-validation with the DOG with gamma model
% In total, this script has 6 models 
% Author - Abhishek De, 11/20
...
NUMMODELS = 6; %  % 1-Crescent, 2-Gabor, 3-DoG, 4-Single Gaussian, 5-Single Gaussian with indepenent variances along x and y, 6- DOG model with independent variance along the x- and y-axis 
SSE_gamma = cell(numcells,NUMMODELS); 
Deviation_gamma = cell(numcells,NUMMODELS);
Peraccuracy_gamma = cell(numcells,NUMMODELS); 
Winning_model_gamma = zeros(numcells,1);
...
for ii = 1:numcells
    ...
    SSE_Crescent = []; SSE_Gabor = []; SSE_DOG = []; SSE_SG = []; SSE_S2G = []; SSE_DOGgamma = [];
    Peraccuracy_Crescent = []; Peraccuracy_Gabor = []; Peraccuracy_DOG = []; Peraccuracy_SG = []; Peraccuracy_S2G = []; Peraccuracy_DOGgamma = [];
    Deviation_Crescent = []; Deviation_Gabor = []; Deviation_DOG = []; Deviation_SG = []; Deviation_S2G = []; Deviation_DOGgamma = [];
    ...
    for jj = 1:c.NumTestSets
        ...
        [out4,fittedSGaussian,Rsquare_SG] = SingleGaussianfit(m); % fitting the Single_Gaussian
        [out5,fittedS2Gaussian,Rsquare_S2G] = SingleGaussian2Dfit(m); % fitting the Single_Gaussian with variable variances

        [out3,fittedDOG,Rsquare_DOG] = DOGfit(m,[]); % Fitting the DoG
       
        [tmp_out1,fittedDOGwgamma1,Rsquare_DOGwgamma1] = DOGfit_wgamma(m,out3); % Fitting the DoGwgamma
        [tmp_out2,fittedDOGwgamma2,Rsquare_DOGwgamma2] = DOGfit_wgamma(m,[]);
        ...
        tS = fittedSGaussian(:)*u1(:,1)'; tS = flipdim(tS(:)*temporal_profile,2); initargsS = {tS(:), 0, sum(stro2.trial(:,nframesidx)), [nstixperside^2 3 maxT]};
        tS2 = fittedS2Gaussian(:)*u1(:,1)'; tS2 = flipdim(tS2(:)*temporal_profile,2); initargsS2 = {tS2(:), 0, sum(stro2.trial(:,nframesidx)), [nstixperside^2 3 maxT]};
        tDgamma = fittedDOGwgamma(:)*u1(:,1)'; tDgamma = flipdim(tDgamma(:)*temporal_profile,2); initargsDgamma = {tDgamma(:), 0, sum(stro2.trial(:,nframesidx)), [nstixperside^2 3 maxT]};
        ...
        outproj_S = getWhtnsStats(stro2,maxT,'STPROJmod',initargsS,spikename); % Single-Gaussian
        projS = outproj_S{1}; spikeS = outproj_S{2}; clear outproj_S;
        rocS = roc(projS(spikeS==0), projS(spikeS==1));
        if rocS<0.5
            rocS = 1-rocS;
        end
        outproj_S2 = getWhtnsStats(stro2,maxT,'STPROJmod',initargsS2,spikename); % Single-Gaussian with multiple variances
        projS2 = outproj_S2{1}; spikeS2 = outproj_S2{2}; clear outproj_S2;
        rocS2 = roc(projS2(spikeS2==0), projS2(spikeS2==1));
        if rocS2<0.5
            rocS2 = 1-rocS2;
        end
        outproj_Dgamma = getWhtnsStats(stro2,maxT,'STPROJmod',initargsDgamma,spikename); % DoG w gamma
        projDgamma = outproj_Dgamma{1}; spikeDgamma = outproj_Dgamma{2}; clear outproj_Dgamma;
        rocDgamma = roc(projDgamma(spikeDgamma==0), projDgamma(spikeDgamma==1));
        if rocDgamma<0.5
            rocDgamma = 1-rocDgamma;
        end
        ...
        Peraccuracy_SG = [Peraccuracy_SG; rocS];
        Peraccuracy_S2G = [Peraccuracy_S2G; rocS2];
        Peraccuracy_DOGgamma = [Peraccuracy_DOGgamma; rocDgamma];
...
%         projSTA = v2(:,1);
        ...
        fittedSGaussian = fittedSGaussian/norm(fittedSGaussian(:));
        fittedS2Gaussian = fittedS2Gaussian/norm(fittedS2Gaussian(:));
        fittedDOGwgamma = fittedDOGwgamma/norm(fittedDOGwgamma(:));
        ...
        SSE_SG = [SSE_SG; min([sum((fittedSGaussian(:)-projSTA(:)).^2) sum((fittedSGaussian(:)+projSTA(:)).^2)])];
        SSE_S2G = [SSE_S2G; min([sum((fittedS2Gaussian(:)-projSTA(:)).^2) sum((fittedS2Gaussian(:)+projSTA(:)).^2)])];
        SSE_DOGgamma = [SSE_DOGgamma; min([sum((fittedDOGwgamma(:)-projSTA(:)).^2) sum((fittedDOGwgamma(:)+projSTA(:)).^2)])];
        ...
        Deviation_SG = [Deviation_SG; acos(abs(corr(fittedSGaussian(:),projSTA(:))))*180/pi];
        Deviation_S2G = [Deviation_S2G; acos(abs(corr(fittedS2Gaussian(:),projSTA(:))))*180/pi];
        Deviation_DOGgamma = [Deviation_DOGgamma; acos(abs(corr(fittedDOGwgamma(:),projSTA(:))))*180/pi];
          
    end
    
    
    SSE_gamma{ii,1} = SSE_Crescent; SSE_gamma{ii,2} = SSE_Gabor; SSE_gamma{ii,3} = SSE_DOG; SSE_gamma{ii,4} = SSE_SG; SSE_gamma{ii,5} = SSE_S2G; SSE_gamma{ii,6} = SSE_DOGgamma;
    Peraccuracy_gamma{ii,1} = Peraccuracy_Crescent; Peraccuracy_gamma{ii,2} = Peraccuracy_Gabor; Peraccuracy_gamma{ii,3} = Peraccuracy_DOG; Peraccuracy_gamma{ii,4} = Peraccuracy_SG; Peraccuracy_gamma{ii,5} = Peraccuracy_S2G; Peraccuracy_gamma{ii,6} = Peraccuracy_DOGgamma; 
    Deviation_gamma{ii,1} = Deviation_Crescent; Deviation_gamma{ii,2} = Deviation_Gabor; Deviation_gamma{ii,3} = Deviation_DOG; Deviation_gamma{ii,4} = Deviation_SG; Deviation_gamma{ii,5} = Deviation_S2G; Deviation_gamma{ii,6} = Deviation_DOGgamma; 
...
saveresults = 1;
if saveresults
    save SSE_gamma SSE_gamma
    save Deviation_gamma Deviation_gamma 
    save Peraccuracy_gamma Peraccuracy_gamma
end
