function [out,fittedSGaussian,R_square,Pearson_R] = SingleGaussianfit(im)
if (size(im,1) ~= size(im,2))
    ...
    numerator = fval; %min([sum((fittedSGaussian(:)+im(:)).^2) sum((fittedSGaussian(:)-im(:)).^2)]);
    denominator = sum(im(:).^2.*(im(:)-mean(im(:))).^2);
    R_square = 1-(numerator/denominator);
    Pearson_R = acos(abs(corr(fittedSGaussian(:),im(:))))*180/pi;
