...
nkt = 100; % 100 ms 
maxT = nkt/dt;
Filt = 0.5*exp(-(0:maxT)).*poisspdf(0:maxT,20); % Filter response of a hypothetical response
Filt(1) = 0;
...
<<<<<<< .mine
%**********************************************************%
% Plotting the Results obtained using fitGLM_AD
figure(plot_counter);
...
% Note - fitting the filter at every time point gives back crazy filter shapes 
