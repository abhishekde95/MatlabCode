...
if plotFlag
    ...
    xlim([1 length(k)])
    set(gca,'xtick',0:round(length(k)/5):length(k),'xticklabel',round(fliplr(-dt*(0:round(length(k)/5):length(k))))); axis square;
    
    subplot(2,2,3)
    plot(k)
    xlim([1 length(k)])
    set(gca,'xtick',0:round(length(k)/5):length(k),'xticklabel',round(fliplr(-dt*(0:round(length(k)/5):length(k)))))
    xlabel('time (ms)'); axis square;
    ...
    xlim([1 length(h)])
    set(gca,'xtick',0:round(length(h)/5):length(h),'xticklabel',round(dt*(0:round(length(h)/5):length(h)))); axis square;
    subplot(2,2,4)
    plot(h)
    xlim([1 length(h)])
    set(gca,'xtick',0:round(length(h)/5):length(h),'xticklabel',round(dt*(0:round(length(h)/5):length(h))))
    xlabel('time (ms)')
    title('post-spike filter'); axis square;
end