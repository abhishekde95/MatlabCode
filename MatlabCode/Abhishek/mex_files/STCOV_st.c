/*****************************************************************/
/*  Function to calculate covariance matrices spatio-temporally, */
...
/* WhiteNoiseOnline.m                                            */
/* GDLH 2/24/08                                                  */
/* Adapted by Abhishek De 6/29/2015 from GDLH                    */                                       
/*****************************************************************/
  ...
  if ((nrhs != 2) | !mxIsCell(SPIKES))
  {
	mexErrMsgTxt("Argument after 'init' should be {npixels, ncolors, nframes}");
      ...
      *p_nframes = (int) *mxGetPr(mxGetCell(SPIKES, 2));
	  ...
	  STCross = mxCalloc((ndims * ndims), sizeof(double)); /* no cross products between frames */
      if (STS && STCross)
	{
	       ...
	       int npixels, int ncolors, int nframes)
{
  ...
  for (i=whichframe-nframes+1;i<=whichframe;i++)
  {
	       ...
	       int npixels, int ncolors, int nframes)
{
  ...
  for (i=0;i<ndims;i++)
  {
     for (j=0;j<=i;j++)
     {
	    ...
	    rgbarr[ndimsperframe*(whichframe-nframes+1)+i]*
	    rgbarr[ndimsperframe*(whichframe-nframes+1)+j]; 
	  }
   }
}
...
{ 
  /* Using 'static' for one time declaration*/
