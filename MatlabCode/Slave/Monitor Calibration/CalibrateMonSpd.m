...
if isempty(g_usebitspp)
    g_usebitspp = input('Which high-res display device? [0=None, 1=Bits++/#, 2=Data/View/ProPixx] ');
    if isempty(g_usebitspp), g_usebitspp = 0; end
    if isempty(g_usecolorpp) && g_usebitspp > 0
        g_usecolorpp = input('Are you in Color++ (Bits++/#) or using C48 mode (Data/View/ProPixx)? [0=No, 1=Yes] ');
        if isempty(g_usecolorpp), g_usecolorpp = 0; end
    end
    switch(g_usebitspp)
        case 0
        ...
        case 2
            if g_usecolorpp
                fprintf('Using Data/View/ProPixx in C48 mode.\n');
            else
                fprintf('Using Data/View/ProPixx in L48 CLUT mode.\n');
            end
        otherwise
...
cal.describe.boxSize = 100;
cal.nDevices = 3;
cal.nPrimaryBases = 1;

whichMeterType = input('Which device are you using to calibrate your display? [0=None, 1=PR650, 2=CVI, 4=PR655, 5=PR670, 6=PR705] ');
if isempty(whichMeterType), whichMeterType = 0; end
...
% Save the structure
fprintf(1, '\nSaving to %s.mat\n', newFileName);
fname = [newFileName(~isspace(newFileName)),'.mat'];
potentialpathtofile = fullfile(filesep,'home','horwitzlab','Desktop','MatlabCode','Slave','Monitor Calibration','Monitor data',newFileName,fname);

if isfile(potentialpathtofile)
    SaveCalFile(cal, newFileName, potentialpathtofile);
else
