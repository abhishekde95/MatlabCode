...
DEFAULTNFRAMES = 6;  % Number of frames to go back before each spike
...
while (~stopnow)
    ...
    % Note: the number of actual screen refreshes is not nframes, it's nframes/stats.nvblsperstimupdate
    % nframes is actually the number of stimulus updates. GDLH 9/9/16.
...
function GetHeader(p)
    ...
    a.stats.nvblsperstimupdate = GetVal(C.NVBLSPERSTIMUPDATECD, p.events);
    a.stats.gotHeader = 1;
    set(gcf,'UserData',a);
end
...
function ClearStats()
    ...
    if (a.stats.nstixperside)
        try
            clear('a.stats.gunnoise.spike')
            for i = 1:TOTALPOSSIBLESPIKES  % preallocating space
                a.stats.gunnoise.spike{i}.STS = single(zeros([3*a.stats.nstixperside^2 nframesback]));
                a.stats.gunnoise.spike{i}.STCross = single(zeros([(3*a.stats.nstixperside^2)^2 nframesback]));
                a.stats.conenoise.spike{i}.STS = single(zeros([3*a.stats.nstixperside^2 nframesback]));
                a.stats.conenoise.spike{i}.STCross = single(zeros([(3*a.stats.nstixperside^2)^2 nframesback]));
            end
        catch ME
            keyboard
        end
...
function SetUpFig(DEFAULTNFRAMES)
    ...
    for i = 1:16
        for j = 1:2
            whichspikestring{length(whichspikestring)+1} = ['chan ',num2str(i),'(',num2str(j),')'];
        end
    end
    clf;
    ...
    a.uicontrols.whichspike = uicontrol('style','popupmenu','String',whichspikestring,'Position',[90 20 70 20]);
    a.uicontrols.projortho = uicontrol('style','popupmenu','String',{'None','PC','STA'},'Position',[170 20 60 20]);
    a.uicontrols.whichnoisetype = uicontrol('style','popupmenu','String',{'gun','cone'},'Position',[240 20 60 20]);
    ...
    function SetUpAxes()
        ...
        a.axeshandles.PC = h2;
        a.axeshandles.text = axes('position',[figpos(3)-200 55 1 1]); % For nspikes
        set(gca,'Visible','off');
...
set(a.axeshandles.subunit, 'UserData',UD);
set(a.uicontrols.nudgestim,'BackgroundColor',[1 .2 .2]);
end
...
%     invM = stats.invM;
    ...
    for i = 1:numel(p.spikes)
        if isempty(p.spikes{i})
            continue
        end
        spiketimes = p.spikes{i}-t_stimon;
        if (any(spiketimes))
            frametimes = linspace(0, nframes*stats.msperframe*stats.nvblsperstimupdate, nframes)+((stats.nvblsperstimupdate*stats.msperframe)/2)';
            ...
            if (numel(out{1}) ~= numel(stats.gunnoise.spike{i}.STS))
                stats.gunnoise.spike{i}.STS = zeros(size(out{1}));
                stats.gunnoise.spike{i}.STCross = zeros(size(out{2}));
                stats.gunnoise.spike{i}.nspikes = 0;
                stats.conenoise.spike{i}.STS = zeros(size(out{1}));
                stats.conenoise.spike{i}.STCross = zeros(size(out{2}));
...
function stopnow = DealWithMessage(msgSize)
    ...
    function nudgerequest() %#ok<DEFNU>
        ...
        if (~isempty(b.nudge))
            ...
            set(a.uicontrols.nudgestim,'BackgroundColor',[.94 .94 .94]);
        end
    end
end
...
function UpdateQueueText(p)
    ...
    nspikes = [size(p.spikes{1,1},1), size(p.spikes{2,1},1)];  % Looking at spikes 1 and 2 on channel 1
    if (nspikes(2) == 0)
    ...
    % commented out code below is for adding all spikes across u-probe
    % channels
    %nspikes = sum(sum(cellfun('length',p.spikes))); % total number of spikes across channels
    %str = ['E: ',num2str(nevents),'  S:',num2str(nspikes)];
...
%     sigma = stats.gunnoise.sigma;
    mu = [.5 .5 .5]'; % GDLH for p;lotting in reasonable colors
    ...
    spikenames = get(controls.whichspike,'String');
    ...
    cla;
    text(0,0,[spikenames{whichspike},' nspikes = ',num2str(n)]);
    ...
    for i = 1:nframesback
        ...
        if (SMALLPC)
            if (strcmp(PROJORTHO,'PC')) % Take penultimate small PC if projecting out STA
                try
                    [tmp,d] = eigs(double(subSTCframe),2,'SR', opts);
                    d = diag(d);
                    tmp = tmp(:,d == max(d));
                catch
                ...
                [tmp,~] = eigs(double(subSTCframe),1,'SM', opts);
            end
        else
            [tmp,~] = eigs(double(subSTCframe),1,'LM', opts);
        end
        v = double(mask);
        v(mask) = tmp;  % Inserting the 0's for the masked pixels
